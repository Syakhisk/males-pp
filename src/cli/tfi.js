...
function start() {
  ...
  let file = fs.readFileSync(appDir + "\\" + argv.b).toString();
  file = file.replace(
    "'.\\src\\tools-for-instagram.js'",
    "'..\\src\\tools-for-instagram.js'"
  );
  file = file.replace(
    '".\\src\\tools-for-instagram.js"',
  ...
  file = file.replace(
    "login()",
    `login({inputLogin: '${argv.u}', inputPassword: '${argv.p}', inputProxy: ${argv.y}})`
  );
  console.log(file);
  (async () => {
    await eval(file);
...
function help() {
  ...
  console.log(
    "Execute bot: tfi start -b bots/botname.js -u username -p password -y proxy "
  );
}

function generateBot(botName) {
  let botContent =
    `require('..\\src\\tools-for-instagram.js');
...
`;
  ...
  fs.mkdirSync(appDir + "\\bots\\", { recursive: true }, (err) => {
    if (err) throw err;
  });
  fs.writeFileSync(appDir + "\\bots\\" + botName + ".js", botContent, {
    flag: "wx",
  });
  console.log("Bot created :" + appDir + "\\bots\\" + botName + ".js");
}
